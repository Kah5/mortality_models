...
spongy.susceptible<- c("chestnut oak", "white oak", "hickory spp.", "northern red oak", "paper birch")
...
plot_main_region_effects<- function(species.group, covar, ymax.spp){
  ...
  if(! covar %in% interaction.terms){  
    ...
    df.species <- marginal_response_df %>% filter(Species %in%  species.group & Covariate %in% covar)
...
for(h in 1:length(sprucefir.top5$Covariate)){
  region.plt.list[[h]] <- plot_main_region_effects(species.group = spruce.fir, 
                                                   covar = sprucefir.top5$Covariate[h], 
                                                   ymax.spp = 0.05)
}
...
for(h in 1:length(spongy.susceptible.top5$Covariate)){
  region.plt.list[[h]] <- plot_main_region_effects(species.group = spongy.susceptible, 
                                                   covar = spongy.susceptible.top5$Covariate[h], 
                                                   ymax.spp = 0.01)
}
...
region.plt.list <-list()
for(h in 1:length(spongy.resist.top5$Covariate)){
  region.plt.list[[h]] <- plot_main_region_effects(species.group = spongy.resist, 
                                                   covar = spongy.resist.top5$Covariate[h], 
                                                   ymax.spp = 0.0075)
}
...
spongy.immune.top5 <- var_summary %>% filter(Species %in% spongy.immune) %>% ungroup()%>%
  ...
  summarise(mean.region = median(mean))%>% arrange(desc(mean.region)) %>% 
  group_by(Species)%>% #View()
  slice(1:8) %>% ungroup() %>% select(Covariate) %>% distinct()

region.plt.list <-list()
for(h in 1:length(spongy.immune.top5$Covariate)){
  region.plt.list[[h]] <- plot_main_region_effects(species.group = spongy.immune, 
                                                   covar = spongy.immune.top5$Covariate[h], 
                                                   ymax.spp = 0.0075)
}
...
spongy.immune.effects <-plot_grid(
  plot_grid(region.plt.list[[4]]+theme(legend.position = "none"),
            region.plt.list[[1]]+theme(legend.position = "none"),
            region.plt.list[[5]]+theme(legend.position = "none"),
            region.plt.list[[7]]+theme(legend.position = "none"),
...
save_plot(paste0(output.folder,"images/spongy.immune_dominant_marginal_variance_effects.png"), 
          spongy.immune.effects, base_width = 16, base_height = 15) 
...
region.plt.list <-list()
for(h in 1:length(mixed.top5$Covariate)){
  region.plt.list[[h]] <- plot_main_region_effects(species.group = mixed, 
                                                   covar = mixed.top5$Covariate[h], 
                                                   ymax.spp = 0.0075)
}
