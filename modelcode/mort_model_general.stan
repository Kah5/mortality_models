...
transformed parameters{
  ...
  //logit_p_annual = alpha_SPP + xM[1:N]*u_beta; //regression
//  p_annual = inv_logit(alpha_SPP + xM[1:N]*u_beta);//annual suvival probability
//  mM[1:N] = p_annual[1:N]^Remper[1:N]; //remper survival probability over remeasurment for bernoulli logit
   mM[1:N] = inv_logit(alpha_SPP + xM[1:N]*u_beta)^Remper[1:N]; //remper survival probability over remeasurment for bernoulli logit
}
model {
